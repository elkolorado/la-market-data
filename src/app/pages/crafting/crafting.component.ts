...
export class CraftingComponent implements OnInit, OnDestroy {
  ...
  timeprofits: { [itemId: string]: Recipe } = {};
  ...
  buildSubMenu(categoryId: string) {
    if (categoryId == 'favorites') {
      ...
      this.submenu = [{
        id: 'all',
        name: "All",
        items: Object.values(this.recommendations).sort((a,b) => b.totalprofit! - a.totalprofit!)
      }];
    } else if (categoryId == 'timeprofits') {
      this.submenu = [{
        id: 'all',
        name: "All",
        items: Object.values(this.timeprofits).sort((a,b) => b.timeprofit! - a.timeprofit!)
      }];
    ...
    this.recipes = craftingdata.reduce<{ [itemId: string]: Recipe }>((acc, cd) => {
      acc[cd.id] = {
        ...
        craftingTimeSeconds: function () {
          const timeMatch = cd.craftingTime.match(/((\d{1,2})h)?[ ]?((\d{1,2})m)?[ ]?((\d{1,2})s)?/);
          return (parseInt(timeMatch?.[2] || '0') * 3600 + parseInt(timeMatch?.[4] || '0') * 60 + parseInt(timeMatch?.[6] || '0')*1);
        }(),
  ...
  setRecipePrices() {
    for (const recipeId in this.recipes) {
      ...
      if (this.marketData[itemId]?.lowPrice) {
        this.recipes[recipeId].price = Math.max(Math.round( 
          ...
          this.recipes[recipeId].amount * 100 ) / 100
        ), 1);
      } else {
      ...
      for (const ingredientIndexStr in this.recipes[recipeId].ingredients) {
        ...
        if (this.marketData[ingredientId]?.lowPrice) {
          this.recipes[recipeId].ingredients[ingredientIndex].price = 
            parseFloat((this.marketData[ingredientId]?.lowPrice /
            this.marketData[ingredientId]?.amount *
            this.recipes[recipeId].ingredients[ingredientIndex].amount!).toFixed(2))
           || 1;
      ...
      if (this.recipes[recipeId].recomendation == 'Craft') {
        ...
        this.recipes[recipeId].totalprofit = this.recipes[recipeId].price! - this.recipes[recipeId].craftTotal!;
        this.recipes[recipeId].timeprofit = this.recipes[recipeId].totalprofit! / ((this.recipes[recipeId].craftingTimeSeconds!) / 60)
        // console.log(this.recipes[recipeId].craftingTimeSeconds)
      ...
      if((this.recipes[recipeId].totalprofit || 0) > 0){
        this.recommendations[recipeId] = this.recipes[recipeId];
        this.timeprofits[recipeId] = this.recipes[recipeId];


      }
      // if (Math.min((this.recipes[recipeId].craftvsbuy || 0), (this.recipes[recipeId].totalprofit || 0)) < -30) {
      //   this.recommendations[recipeId] = this.recipes[recipeId];
      // }
    }
    ...
    if (this.filter.category == 'timeprofits') {
      this.buildSubMenu('timeprofits');
      if (this.selectedRecipeId) {
        // console.log(this.selectedRecipeId);
        if (this.recipes[this.selectedRecipeId].craftvsbuy! > -20) {
          this.selectedRecipeId = undefined;
          this.router.navigate([this.common.regionSlug, 'crafting', 'timeprofits', 'all', this.submenu[0].items[0].id], { queryParamsHandling: 'preserve' });
          this.selectedRecipeId = this.submenu[0].items[0].id;
        }
      } else {
        this.router.navigate([this.common.regionSlug, 'crafting', 'timeprofits', 'all', this.submenu[0].items[0].id], { queryParamsHandling: 'preserve' });
  ...
  updateDiscounts() {
    ...
    for (const recipeIndex in this.recipes) {
      ...
      if((this.recipes[recipeIndex].totalprofit || 0) > 0){
        this.recommendations[recipeIndex] = this.recipes[recipeIndex];
        this.timeprofits[recipeIndex] = this.recipes[recipeIndex];


      }
      // if (Math.min((this.recipes[recipeIndex].craftvsbuy || 0), (this.recipes[recipeIndex].totalprofit || 0)) < -30) {
      //   this.recommendations[recipeIndex] = this.recipes[recipeIndex];
      // }
    }
    if (this.filter.category == 'recommendations') {
    ...
    for (let ingredientIndex in this.recipes[recipeIndex].ingredients) {
      ...
      if (this.recipes[recipeIndex].ingredients[ingredientIndex].ingredients) {
        ...
        if (this.recipes[recipeIndex].ingredients[ingredientIndex].craftTotal && this.recipes[recipeIndex].ingredients[ingredientIndex].price) {
          this.recipes[this.selectedRecipeId!]!.totalprofit = this.recipes[this.selectedRecipeId!]!.price! - this.recipes[this.selectedRecipeId!]!.craftTotal!
